# Report Template

Use this template for full research. The report serves two audiences at once:

- **正文（第 0–10 节）**：一个不懂该产业的读者按顺序读完，应当能说清产业链、利益传导、代表企业、周期位置、资金动向和未来资金可能走向。
- **深读内容（每节的"进阶视角"块 + 第 8 节）**：有产业或投资经验的读者应当能读到非共识判断、口径陷阱、与上一轮周期的对照和冲突证据。
- **附录（A/B/C）**：审计层。证据台账、数据时效、就绪度与执行记录全部放这里，不得占据正文。

Keep gaps explicit; delete unused optional rows rather than filling them with generic prose. 表头、栏目一律用中文；证据编号保持 E1/E2 格式。禁止任何一句解释性文字在报告内复用三次及以上。

````markdown
# [行业]行业供需周期分析

分析日期：[YYYY-MM-DD HH:mm:ss 时区]
地理范围：[范围]
数据时效：[最新整期实际值；部分月度实际值；指引与预测分开说明]
行业边界：[纳入什么、排除什么]
研究模式：完整深研

> 阅读路线——第一次接触本行业：按 0→1→2→3→4→5→6→7 顺序读，遇到不认识的词查第 10 节术语表。有经验的读者：读 0 后直接跳 4、5 的进阶视角和 8，再用附录A核对证据。

## 0. 一页看懂

**这个行业是做什么的**：[两三句人话：产品是什么、卖给谁、大概多大规模。不许堆行话；首次出现的术语必须收进第 10 节术语表。]

**一句话判断**：[周期假设 + 核心矛盾 + 最大证据缺口]

- 周期阶段：[阶段名]（推导见第 5 节）
- 结论状态：[可发布 / 暂定 / 阶段待验证]
- 置信度：[高 / 中 / 低]
- 证据截至时间：[YYYY-MM-DD HH:mm:ss 时区]
- 上调条件：[能让结论更强的可度量条件]
- 下调条件：[会让结论降级或失效的可度量条件]
- 最大缺口：[gap]

**三个最重要的数字**：

| 数字 | 含义 | 为什么它最重要 | 证据 |
|---|---|---|---|
|  |  |  | E_ |

## 1. 产业链地图

### 1.1 全景图

```mermaid
[必须包含：主链各环节、平行输入（设备、材料、软件、能源、资金等，按实际情况取舍）、最终付款方。
禁止把平行输入画成串行链；禁止把产品（如"焦炭"）当作环节节点。]
```

[图下最多五句话讲解：货往哪个方向走、钱从哪里来、当前瓶颈卡在哪个节点。]

### 1.2 环节详解

至少写 4 个关键环节；对每个重要环节写一个小节（#### 1.2.x 环节名），每节必须包含：

**它是干什么的**：[用外行能懂的话写出具体动作和产出，例如"把设计图变成硅片上的电路"。禁止"负责把上游投入转成下游可采购、可验证的产品或服务"这类放之四海皆准的循环定义。]

**向谁采购**：[点名具体物料、设备、能源、软件、服务或资本来源；不要把上下游合在同一句]

**卖给谁**：[点名直接买方和最终预算方；直接买方与最终付款方不同时都要说明]

**代表企业**：

| 公司 | 上市地/代码 | 在该环节的地位 | 产能/生产控制方式 | 为什么能代表该环节 | 证据 |
|---|---|---|---|---|---|
|  | [交易所+代码 / 未上市 / 机构] |  | [自有 / 合资及权益比例 / 委外 / 长协锁量 / 市场采购，并说明实际控制边界] |  | E_ |
|  | [交易所+代码 / 未上市 / 机构] |  | [不得只写“自产”或“外采”；写清技术、良率、扩产或产量分配由谁控制] |  | E_ |

**怎么赚钱、议价能力**：[收费方式；议价强弱及其原因（稀缺性/认证周期/转换成本），不许只写结论]

**为什么会卡住**：[指出该环节形成有效供给前最具体的约束，例如良率、认证、交付、品位、并网、库存或客户预算；不得复制“它是干什么的”]

**进阶视角**：[至少 36 个有效字符；说明该环节的核心壁垒、当前争议和本报告立场，并附证据 ID。写不出争议就写“未检索到分歧”并说明查过哪里。]

### 1.3 钱怎么流：利益传导

| 问题 | 回答（必须点名具体环节和企业，禁止通用套话） | 证据 | 缺口 |
|---|---|---|---|
| 谁最终付款？ |  |  |  |
| 付款方资金来源与预算持续性 | [区分经营现金流、存量现金、债务、股权或财政资金；写明预算收紧的可观察条件，不伪造“还能烧几年”] |  |  |
| 利润当前集中在哪个环节，为什么？ |  |  |  |
| 谁承担资本开支和库存风险？ |  |  |  |
| 谁有定价权，凭什么？ |  |  |  |
| 谁重要但赚不到钱？ |  |  |  |

订单与预算流（从右到左的钱，如何变成从左到右的货）：

```text
[终端真实需求] -> [预算所有者] -> [系统/整机订单] -> [部件/工序订单] -> [产能与设备订单]
```

## 2. 需求：谁在买、为什么买

### 2.1 政策重要性闸门

- 政策是否实质驱动当前周期：[是 / 否]
- 判断依据：[一句话说明政策是否改变当前需求、有效供给、价格与利润、成本、贸易流、资本开支、融资或准入，并附证据 ID]
- 主要作用通道：[需求 / 供给 / 价格与利润 / 成本 / 贸易流 / 资本开支 / 融资与资本准入 / 认证与经营准入，可多选]
- 政策状态截至：[YYYY-MM-DD HH:mm:ss 时区]

若答案为`是`，只列实质相关的国家/地区，并填写以下表格；若答案为`否`，删除整张表，不填 `N/A`：

| 国家/地区 | 政策或工具 | 状态与截至日期 | 影响环节 | 可核实经济效应 | 落地差或局限 | 到期/反转风险 | 证据 |
|---|---|---|---|---|---|---|---|
|  |  | 已立法/已发布规则/已执行/已拨付，截至 YYYY-MM-DD |  |  |  |  | E_ |

事实（每条带证据编号、期间、地域）：

- [E_ + 指标 + 期间 + 地域 + 来源]

| 终端用途 | 买方/预算所有者 | 购买动因 | 已兑现还是预期 | 可观察指标 | 证据 |
|---|---|---|---|---|---|
|  |  |  |  |  |  |

推断与假设：

- [推断，注明由哪些证据推出]
- [假设 + 什么现象会推翻它]

**进阶视角**：[需求端最容易被误读的口径（如出货≠使用、补库≠终端好转）；当前需求数据里最可疑的一个点。]

## 3. 供给：现在有多少、真能用的有多少

| 环节/项目 | 公告产能 | 已安装 | 已验证/爬坡达标 | 订单支撑、性质与可撤销性 | 释放窗口 | 证据 | 缺口 |
|---|---:|---:|---:|---:|---|---|---|
|  |  |  |  | [预付款 / 长协 / take-or-pay / 普通可撤销订单 / 框架意向 / 条款未披露；同时检查重复或多头下单] |  |  |  |

事实：

- [产能、良率、认证、交期或资本开支的具体数字]

推断与假设：

- [推断]
- [假设 + 反证条件]

**进阶视角**：[名义产能与有效产能之间当前最大的折损环节在哪，依据是什么；哪些扩产公告值得打折看待。]

## 4. 供需矛盾与高频信号

核心矛盾（用具体名词回答，不许套模板句）：

- 什么东西短缺或过剩？在哪个环节、什么口径下？
- 能持续多久？谁能解决、合格产能什么时候到？

| 信号 | 最新值/方向 | 数据期间 | 证据 | 解读（这个数说明了什么） | 缺口 |
|---|---|---|---|---|---|
| 价格 |  |  |  |  |  |
| 订单 |  |  |  |  |  |
| 库存 |  |  |  |  |  |
| 利用率/良率 |  |  |  |  |  |
| 利润/现金流 |  |  |  |  |  |

数据期间必须写具体年月或季度，禁止写"最新已披露期"。若某行无公开可比数据，写明缺口原因和下一个应监控的来源。

### 4.1 公开渠道代理（适用时）

只有在存在可重复观察的公开零售、分销或招标渠道时保留本节；不适用则整节删除。渠道页面只能证明该卖家、SKU、价格口径和时点的现象，单一渠道不能独立证明上游涨价、全行业短缺或终端需求爆发。

| 平台/渠道 | SKU/规格 | 卖家/主体 | 价格口径 | 观察时间 | 可得性 | 对比基线 | 证据 | 局限 | 交叉验证 |
|---|---|---|---|---|---|---|---|---|---|
|  |  |  | [挂牌价 / 券后价 / 成交价] | [YYYY-MM-DD HH:mm 时区] | [有货 / 缺货 / 下架 / 交期] | [同SKU、同卖家、同口径历史观察] | E_ | [促销、清仓、换代、第三方卖家等限制] | [另一渠道或上游价格/库存证据 E_] |

## 5. 周期位置与传导

传导链（本行业专属版本，不是通用模板）：

```text
[需求/预算] -> [订单] -> [收入确认] -> [瓶颈环节利润] -> [资本开支] -> [合格产能] -> [价格/利润逆转或持续短缺]
```

| 阶段/日期 | 性质（已发生/计划/预测/风险窗口） | 信号 | 利润池往哪移 | 关键时滞 | 证据 | 下一步验证 |
|---|---|---|---|---|---|---|
|  |  |  |  |  |  |  |

当前阶段：

- 阶段：[阶段名，或"阶段待验证"]
- 进入时间/锚点：[日期 + 锚定哪期实际数据]
- 预期切换条件：[可度量的触发条件，不是"未来2-6个季度"这类泛泛时间窗]
- 置信度：[高/中/低]
- 什么会证明这个判断错了：[可度量条件]

### 5.1 资本周期交叉校验（资本周期、行业上坡/下坡或风口拐点问题必须保留；否则可删除）

- 观察节点：[必须是具体产业链环节，不能只写整个“AI/计算机/半导体行业”]
- 资本开支方向：[方向 + 明确期间 + 证据 ID]
- 资本开支口径：[现金支付 / 资产增加 / 租赁 / 公司指引；主体、财年口径和包含项目]
- 资本开支性质：[增长性 / 维护性 / 替换性 / 混合 / 未拆分]
- 回报代理：[ROIC / 毛利率 / 经营利润率 / 自由现金流 / 单位利润 / 人均指标；写明期间与证据 ID]
- 口径可比性：[哪些可比；不可比时写清分子、分母、统计范围或财年差异]
- 供给兑现时滞：[资本开支到安装、认证、良率爬坡和客户支持产出的路径与时长]
- 交叉判断：[资本开支方向 × 回报方向支持哪种假设；价格、订单、库存、利用率是否一致]
- 反证条件：[会推翻该交叉判断的数值或事件条件]

**进阶视角：与上一轮周期的对照**：[上一轮同类周期（写明起止年份）中，从利润兑现到产能过剩用了多久、拐点信号是什么；这一轮哪里相似、哪里不同；样本只有一轮或不可比时如实说明。]

## 6. 资金动向

先做规定动作再下结论。检索尝试记录（做过什么就写什么，禁止跳过本表直接写缺口）：

| 尝试的来源类型 | 具体来源 | 结果（拿到数据 / 无公开数据 / 口径不可比） |
|---|---|---|
| 行业指数估值分位 |  |  |
| 行业 ETF 份额/资金流 |  |  |
| 北向/两融或同类资金流指标（适用时） |  |  |
| 龙头股价与盈利的剪刀差 |  |  |

分层代理证据（不能要求一个工具覆盖整条产业链，也不能把覆盖不完整直接写成“无数据”）：

| 代理层级（行业/子链/公司） | 工具/主体 | 覆盖节点 | 指标与期间 | 来源 | 结论 | 局限 |
|---|---|---|---|---|---|---|
|  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |

拿到数据时填写：

| 产业现实 | 市场叙事/定价证据 | 预期阶段（未定价/扩散/已定价/消化/反转） | 来源 | 解读 |
|---|---|---|---|---|
|  |  |  |  |  |

无论数据是否齐备，必须写定性判断段落：

- 估值口径校准：[说明当前PE/估值是否受利润高点或低点扭曲；至少结合利润率与价格、库存或利用率中的一个周期指标。PE不可比、亏损或数据缺口也要说明原因和替代口径。]
- 市场当前大概率**已定价**：[什么]
- 市场当前大概率**未定价**：[什么]
- 判断依据与不确定性：[基于哪些间接证据；哪里可能错]

### 6.4 隐含预期反推

仅在取得同日期可比的市场价格与盈利/现金流基础时填写。目标是反推当前价格要求什么经营路径成立，不是预测未来股价。

| 反推方法 | 已知输入 | 关键假设 | 反推出的门槛 | 产业证据对照 | 证据与局限 |
|---|---|---|---|---|---|
| PE桥接 / 反向DCF / 其他 | [价格日期、业绩或现金流期间、股本/净债务] | [期限、必要回报率、退出倍数或终值增长] | [EPS/FCF/利润率/利用率/ROIC 所需路径] | [产业证据支持 / 要求偏高 / 证据不足，并说明对应条件] | [证据ID；标明情景推算、口径和缺口] |

若缺少同日期价格、可比盈利/现金流、股本或净债务等关键输入，删除空表并写：`公开数据缺口：[缺什么、尝试过哪些原始来源、下一步监测来源]`。不得把“股价不变后未来 PE 下降”单独解释为低估，也不得输出目标价或“大概率涨到”的价格预测。

若定价数据缺口存在，结论状态保持暂定，并在附录C记录缺口。

## 7. 未来资金可能流向

> 本节是周期传导的情景推演，不构成任何买卖建议、目标价或个股推荐。

| 情景 | 触发条件 | 利润池往哪个环节移动 | 先受益的环节 | 后受益/受损的环节 | 需要盯的证据 |
|---|---|---|---|---|---|
| 基准 |  |  |  |  |  |
| 上行 |  |  |  |  |  |
| 下行 |  |  |  |  |  |

[表后用一段话解释推演逻辑：为什么这个情景下钱会先流向该环节——从订单、交期、认证壁垒和产能弹性推，不从股价走势推。]

## 8. 分歧与反证

主流叙事 vs 本报告：

| 市场主流叙事 | 本报告判断 | 分歧在哪 | 谁的证据更硬 | 证据 |
|---|---|---|---|---|
|  |  |  |  |  |

冲突证据（保留矛盾，不做算术平均）：

| 议题 | 支持证据 | 反对证据 | 口径差异 | 处理 |
|---|---|---|---|---|
|  |  |  |  | 已解决 / 未解决 |

反证优先级：先看实际值，再看指引，最后看预测；公司口径与行业口径冲突时保留差异。

## 9. 观察哨与跟踪

| 指标（写指标名，不是数值） | 基线（数值+日期） | 来源 | 频率 | 正向触发 | 反证触发 | 含义 |
|---|---|---|---|---|---|---|
|  |  | [具体来源或URL] | 月度/季度/事件 | [数值或事件阈值] | [数值或事件阈值] | [这个指标验证第几节的哪个判断] |

基线必须是分析日期时点最新可得的实际值；若最新可得值距分析日期超过一个正常发布周期，注明"数据滞后"及原因。

### 9.1 可比时间序列

仅收同一指标、同一定义、同一单位、同一地域、同一主体的实际值。优先三期以上，两期是底线。预测值、不同公司、不同口径分开列，不造合成指数。

| 日期 | 指标 | 数值 | 单位 | 来源 | 含义 |
|---|---|---:|---|---|---|
|  |  |  |  | [证据编号或具体来源] | [这一条序列能代表什么、不能代表什么] |

若不存在可比公开序列，用一句话替代本表：`可比时间序列缺口：因[原因]不可得；下一步监控[具体来源]。`不许编造数据点或拼接异类指标。

跟踪数据库：

| 日期 | 指标 | 环节 | 数值 | 同比/环比 | 方向 | 来源 | 对判断的影响 | 备注 |
|---|---|---|---:|---:|---|---|---|---|
|  |  |  |  |  |  |  |  |  |

## 10. 术语表

正文首次出现的行业术语、缩写全部收录，按出现顺序排列。

| 术语 | 人话解释（一两句，外行能懂） |
|---|---|
|  |  |

## 附录A 证据台账

| 证据ID | 结论 | 类型 | 发布方 | 发布日期 | 访问日期 | 数据期间 | 地域/单位 | 原文链接/定位 | 已打开 | 时效 | 局限 |
|---|---|---|---|---|---|---|---|---|---|---|---|
| E1 |  | 事实/指引/预测/推断 |  |  |  |  | [该条数据自身的地域和单位，不是整篇报告的地理范围] | [URL + 页/表/节] | 是/否 | 当前/被更新版取代/未核验 | [该条证据自身的局限，逐条写，不许复用同一句] |

## 附录B 数据时效与证据覆盖

| 指标 | 期间 | 状态 | 发布日期 | 访问日期 | 时效 | 来源 | 定位 | 局限 |
|---|---|---|---|---|---|---|---|---|
|  |  | 实际/部分实际/指引/预测 |  |  | 当前/被取代/未核验 |  | [页/表/节] |  |

发布状态说明：

- [已发布什么]
- [尚未发布什么]
- [哪个新来源取代了哪个旧来源]

## 附录C 证据就绪度与研究执行记录

| 证据车道 | 状态 | 已打开可靠来源数 | 最低要求 | 证据/缺口 |
|---|---|---:|---:|---|
| 产业链 | 就绪/缺口 |  | 2 |  |
| 需求 | 就绪/缺口 |  | 3 |  |
| 供给与有效产能 | 就绪/缺口 |  | 3 |  |
| 价格/订单/库存/利润 | 就绪/缺口 |  | 3（或明确缺口） |  |
| 资本市场预期 | 就绪/缺口 |  | 2（或明确缺口+第6节定性段） |  |

| 子任务 | 检索轮次 | 实际使用的路径 | 证据 | 状态 | 缺口/回退 |
|---|---:|---|---|---|---|
|  |  |  |  | 完成/缺口 |  |

事实、推断、假设分层：

- 事实：[证据编号 + 有来源的事实]
- 推断：[由哪些证据推出]
- 假设：[假设 + 反证条件]

## 尾注

- 供需缺口 ≠ 股价上涨。
- 方向正确 ≠ 时点正确。
- 盈利兑现 ≠ 股价继续上涨。
- AI 回答和搜索摘要不是事实。
- 过期数据不是当前事实。
````
